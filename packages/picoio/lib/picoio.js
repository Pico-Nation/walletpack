...
export const picojsUtil = getPicojsApi();
...
export default class PICO extends Plugin {

	constructor(){ super(Blockchains.PICOIO, PluginTypes.BLOCKCHAIN_SUPPORT) }
	...
	getSignablePicojs(accounts, reject, prompt = true, signatureProvider = null){
		const isSingleAccount = accounts instanceof Account;
	...
	returnableAccount(account){ return { name:account.name, authority:account.authority, publicKey:account.publicKey, blockchain:Blockchains.PICOIO }}

	contractPlaceholder(){ return 'picoio.token'; }
	...
	async proxyVote(account, proxyAccount, prompt = false){
		return new Promise(async (resolve, reject) => {
			...
			const pico = this.getSignablePicojs(account, reject, prompt);
	...
	async changePermissions(account, keys){
		if(!keys) return;
		return new Promise(async (resolve, reject) => {
			const pico = this.getSignablePicojs(account, reject);
			...
			return pico.transact({actions},{
				...
				.then(async res => {
					...
					const addAccount = async (keypair, authority) => {
						...
						acc.publicKey = keypair.publicKeys.find(x => x.blockchain === Blockchains.PICOIO).key,
	...
	getImportableAccounts(keypair, network){
		return new Promise((resolve, reject) => {
			let publicKey = keypair.publicKeys.find(x => x.blockchain === Blockchains.PICOIO);
	...
	privateToPublic(privateKey, prefix = null){ try {
		return ecc.PrivateKey(privateKey).toPublic().toString(prefix ? prefix : Blockchains.PICOIO.toUpperCase());
	...
	validPublicKey(publicKey, prefix = null){
		try {
			return ecc.PublicKey.fromStringOrThrow(publicKey, prefix ? prefix : Blockchains.PICOIO.toUpperCase());
		} catch(e){
			return false;
		}
	}
	...
	async balancesFor(account, tokens, fallback = false){
		if(!fallback && await LightAPI.networkString(account.network())){
			...
			const blacklist = StoreService.get().state.scatter.settings.blacklistTokens.filter(x => x.blockchain === Blockchains.PICOIO).map(x => x.unique());
	...
	defaultToken(){ return new Token(Blockchains.PICOIO, 'picoio.token', 'PICO', 'PICO', this.defaultDecimals(), MAINNET_CHAIN_ID) }
	...
	async createAccount(account, name, owner, active, picoUsed){
		return new Promise(async (resolve, reject) => {


			const coreSymbol = account.network().systemToken().symbol;

			const net = (picoUsed/4).toFixed(account.network().systemToken().decimals);
			const cpu = (picoUsed-net).toFixed(account.network().systemToken().decimals);
	...
	async transfer({account, to, amount, token, memo, promptForSignature = true}){
		...
		return new Promise(async (resolve, reject) => {
			const pico = this.getSignablePicojs(account, reject, promptForSignature);

			const result = await pico.transact({
				actions:[{
					account: contract,
					name:'transfer',
					authorization: [{
	...
	async fetchAbis(network, contracts, fallbackToChain = false){
		...
		try {
			return await Promise.all(contracts.map(async account => {
				...
				const abi = picojsUtil.rawAbiToJson(rawAbi);
	...
	async parsePicojsRequest(payload, network){
		try {
			const {transaction} = payload;
			const api = getPicojsApi();
	...
	async parsePicojs2Request(payload, network){
		const {transaction} = payload;
	...
	async requestParser(payload, network){
		if(payload.transaction.hasOwnProperty('serializedTransaction'))
			return this.parsePicojs2Request(payload, network);
		else return this.parsePicojsRequest(payload, network);
	}
}
